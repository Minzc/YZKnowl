        ...
        for kw in kws:
            ...
            if len(obj_poss) != 0:
                ...
                for obj_pos in obj_poss:
                    if kws[obj_pos].sntnc == kws.sntnc:
                        if abs(kws[obj_pos].wrdpos - kws.wrdpos) < 3:
                            type = LESS_THAN_THREE_WORDS
                        else:
                            type = MORE_THAN_THREE_WORDS
                    elif abs(kws[obj_pos].sntnc - kw.sntnc) < 2:
                        type = LESS_THAN_ONE_SENT
            ...
            # P(c=dis|obj-kw)
            if kw_pair_dis[obj_name].has_key(kws.token.word):
                likelihood *= kw_pair_dis[obj_name][kws.token.word][type]
            else:
                likelihood *= 0.25
            if DEBUG:
                print 'keyword:',kws.token.word,likelihood,type
            # For each keyword, go through all words in the sub-sentence to mine best sentiment keywords
            max_likelihood = -1
            best_fit_senti = ''
            for senti_kw in kws:
                if kw == senti_kw or senti_kw.flag == 'obj':
                    continue
                # Only classify words in sentiment dictionary or adjective
                if senti_kw.token.word in snt_dic or senti_kw.token.flag == 'a':
                    snt_lkhd = 1
                    kw_pair_dis.setdefault(kw.token.word,{})
                    snt_pair.setdefault(senti_kw.token.word,{})

                    # P(c=dic|kw-sentiment)
                    if kw_pair_dis[senti_kw.token.word].has_key(kw.token.word):
                        snt_lkhd *= kw_pair_dis[senti_kw.token.word][kw.token.word][type]
                    else:
                        snt_lkhd *= 0.25
                    if DEBUG:
                        print 'pair:',senti_kw.token.word,snt_lkhd,
                    # P(kw-sentiment)
                    if snt_pair[kw.token.word].has_key(senti_kw.token.word):
                        snt_lkhd = snt_lkhd*snt_pair[kw.token.word][senti_kw.token.word]
                    else:
                ...
                # P(keyword)
                if kw_dis.has_key(kw.token.word):
                    likelihood *= kw_dis[kw.token.word]
                else:
                    # If the keyword did not exist in train data set, assign a prior probability
                    likelihood *= 0.000000000001
                if DEBUG:
                    print 'keyword final score:' + kw.token.word,likelihood,kw_dis.get(kw.token.word,0.000000000001)
                if max_likelihood != -1:
                    can_rst[kw.token.word+'$'+best_fit_senti] = likelihood
                else:
